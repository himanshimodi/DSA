...
public:
    int carFleet(int target, vector<int>& position, vector<int>& speed) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            vec.push_back(
                {position[i], float(target - position[i]) / speed[i]});
        }
        ...
        for (auto it : vec) {
            ...
            while (!st.empty() &&
                   (time >= st.top())) { // curr car time >= st.top->car->time
                // becomes a car fleet and now both cars have to go w the speed
                // of the slower car
                //  and both take time of the slower car to reach the
                //  destination
                st.pop();
            }
            st.push(time);
